...
class test_folder_functions(unittest.TestCase):
    ...
    def test_make_directory_exception(self):
        date_object = date.today()
        year = date_object.strftime("%Y")
        directory_to__files: str = "data"
        file_directory = f"{directory_to__files}/tmp-test-{year}"
    ...
    def test_get_directory_list_error(self):
        test_1 = get_directory_list("data")
        assert "error" not in test_1
    ...
    #     test_1 = get_directory_list("bob")
        
    #     assert "error" not in test_1
        
        # self.task1 = mock.MagicMock(get_directory_list("data"))
